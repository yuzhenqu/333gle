...
static void GetPortAndPath(int argc,
  ...
  // check if port number (argv[1]) is reasonable
  *port = atoi(argv[1]);
  if (*port < 1024) {
  ...
  // check if path (argv[2]) is a readable directory
  struct stat dirstat;
  ...
  *path = string(argv[2]);
  ...
  // check the rest of the command line arguments to see if there is at least one readable 
  // index file
