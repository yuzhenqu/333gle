...
bool IsPathSafe(const string& root_dir, const string& test_file) {
 ...
 char root_path[PATH_MAX], file_path[PATH_MAX];
  if (!realpath(root_dir.c_str(), root_path) || 
  !realpath(test_file.c_str(), file_path)) {
    return false;
  }

  if (file_path[strlen(root_path)] != '/') {
    return false;
  }

  // Make sure test_file is a prefix of root_dir.
  if (strlen(file_path) <= strlen(root_path)) {
    return false;
  }

  if (strncmp(root_path, file_path,
              strlen(root_path)) != 0) {
                return false;
  }
